...
from infrastructure.storage.mappers.procedure import ProcedureMapper, ProcedureUsageMapper, LaboratoryProcedureMapper
...
class ProcedureRepositoryImpl(ProcedureRepository):
    ...
    async def find_by_laboratory_procedure(self, laboratory_id: LaboratoryId) -> List[LaboratoryProcedure]:
        ...
        return [LaboratoryProcedureMapper.to_domain(model) for model in models]

    async def find_by_laboratory(self, laboratory_id: LaboratoryId) -> List[Procedure]:
        stmt = select(ProcedureModel).join(
